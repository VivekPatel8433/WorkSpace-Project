...
function filterResults() {
  ...
  const query = sanitizeInput(document.getElementById("searchInput").value.toLowerCase());
  const location = sanitizeInput(document.getElementById("location").value);
  
  // Address/Neighborhood filter
  const addressFilter = sanitizeInput(document.getElementById("addressFilter").value.toLowerCase());
  
  // Square feet filter
  const minSqFt = sanitizeNumber(document.getElementById("minSqFt").value);
  const maxSqFt = sanitizeNumber(document.getElementById("maxSqFt").value) || Infinity;
  
  // Capacity filter
  const capacityFilter = sanitizeNumber(document.getElementById("capacityFilter").value);
  
  // Price filter
  const minPrice = sanitizeNumber(document.getElementById("minPrice").value);
  const maxPrice = sanitizeNumber(document.getElementById("maxPrice").value) || Infinity;
  
  // Availability date filter
  const availabilityDate = validateDate(document.getElementById("availabilityDate").value);
  ...
  const publicTransportChecked = document.querySelector('input[name="publicTransport"]').checked;
  const smokingChecked = document.querySelector('input[name="smoking"]').checked;
  ...
  const results = mockData.filter(item => {
    ...
    // Price filter
    const matchPrice = item.pricePerDay >= minPrice && item.pricePerDay <= maxPrice;
    
    // Availability date filter
    const matchAvailability = !availabilityDate || 
    ...
    const matchPublicTransport = !publicTransportChecked || item.amenities.includes("publicTransport");
    const matchSmoking = !smokingChecked || item.smokingAllowed || item.amenities.includes("smoking");

    return matchText && matchType && matchLocation && matchAddress && 
           matchSqFt && matchCapacity && matchPrice && matchAvailability && 
           matchLeaseTerm && matchWifi && matchParking && matchKitchen && 
           matchMeeting && matchPublicTransport && matchSmoking;
  });
...
function renderResults(data) {
  ...
  if (data.length === 0) {
    resultsContainer.innerHTML = "<p class='text-center text-gray-500 py-8'>No workspaces match your search criteria.</p>";
    return;
  }
  ...
  data.forEach(item => {
    ...
    card.className = "bg-white shadow-lg rounded-lg p-6 transition hover:scale-[1.02] cursor-pointer w-full";
    ...
    const sanitizedAmenities = item.amenities.map(amenity => sanitizeHTML(amenity)).join(", ");
    const sanitizedLeaseTerms = item.leaseTerms.map(term => sanitizeHTML(term)).join(", ");
      ...
      <div class="flex flex-col lg:flex-row gap-6">
        ...
        <div class="lg:w-2/3 space-y-4">
          ...
          <div class="grid grid-cols-2 lg:grid-cols-3 gap-4 text-sm text-gray-600">
            ...
            <p><strong>Available:</strong> ${sanitizedAvailableFrom ? new Date(sanitizedAvailableFrom).toLocaleDateString() : 'N/A'}</p>
...
function showWorkspaceDetails(workspaceId) {
  ...
  const sanitizedLeaseTerms = workspace.leaseTerms.map(term => sanitizeHTML(term)).join(", ");
  const sanitizedAmenities = workspace.amenities.map(amenity => sanitizeHTML(amenity));
    ...
    <div class="space-y-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div class="space-y-4">
          ...
          <div class="space-y-2 text-sm">
            ...
            <p><strong>Available From:</strong> ${sanitizedAvailableFrom ? new Date(sanitizedAvailableFrom).toLocaleDateString() : 'N/A'}</p>
            <p><strong>Lease Terms:</strong> ${sanitizedLeaseTerms}</p>
        ...
        <div class="space-y-4">
          ...
          <div class="bg-gray-50 p-4 rounded-md space-y-2 text-sm">
            ...
            <p><strong>Email:</strong> ${sanitizedOwnerEmail ? `<a href="mailto:${sanitizedOwnerEmail}" class="text-gray-800 hover:underline">${sanitizedOwnerEmail}</a>` : 'N/A'}</p>
            <p><strong>Phone:</strong> ${sanitizedOwnerPhone ? `<a href="tel:${sanitizedOwnerPhone}" class="text-gray-800 hover:underline">${sanitizedOwnerPhone}</a>` : 'N/A'}</p>
          </div>
      ...
      <div class="space-y-2">
        ...
        <div class="flex flex-wrap gap-2">
          ${sanitizedAmenities.map(amenity => 
...
function contactOwner(email, workspaceName) {
  ...
  window.location.href = `mailto:${sanitizedEmail}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
}
...
function showOnMap(workspaceId) {
  ...
  if (workspace && workspace.lat && workspace.lng) {
    ...
    const marker = markers.find(m => {
      ...
      return Math.abs(markerLatLng.lat - workspace.lat) < 0.001 && 
